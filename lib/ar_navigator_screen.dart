...
class NavigationScreen extends StatefulWidget {
  ...
  const NavigationScreen({super.key, this.cameras});
  ...
  void _initCamera() async {
    try {
      ...
      if (widget.cameras != null && widget.cameras!.isNotEmpty) {
        cam = widget.cameras!.first;
      } else {
        final cams = await availableCameras();
        if (cams.isNotEmpty) cam = cams.first;
      }

      if (cam == null) {
        // No camera available on this platform/device
        return;
      }
      ...
      // Camera plugin not available on this platform/build
      _handleError("Camera plugin not available: ${e.message}");
    } on PlatformException catch (e) {
      _handleError("Camera platform error: ${e.message}");
    } catch (e) {
